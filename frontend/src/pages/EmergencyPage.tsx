...
export const EmergencyPage: React.FC = () => {
  ...
  return (
    <div className="max-w-4xl mx-auto py-8 px-4 space-y-8 bg-white min-h-[85vh] text-slate-900">
      <SaaSCard className={`p-8 text-center space-y-6 border transition-all bg-white ${
        sosSent ? 'bg-rose-50 border-rose-400' : 'border-slate-200'
      }`}>
        ...
        <div className="py-4">
          {!sosSent ? (
            <button
              ...
              className="w-44 h-44 rounded-full bg-rose-600 hover:bg-rose-700 text-white font-black text-2xl shadow-2xl shadow-rose-600/30 hover:scale-105 active:scale-95 transition-all flex flex-col items-center justify-center mx-auto border-4 border-white animate-sos cursor-pointer"
            >
      ...
      <div className="space-y-4">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          {emergencyServices.map((srv, idx) => (
            <SaaSCard key={idx} className="p-5 rounded-2xl border border-slate-200 space-y-3 flex flex-col justify-between bg-white">
              <div>
                <span className="px-2.5 py-0.5 rounded text-[10px] font-bold bg-rose-100 text-rose-800 border border-rose-200">
                  {srv.type}
                </span>
                <h4 className="text-base font-extrabold text-slate-900 mt-2">{srv.name}</h4>
                <p className="text-xs text-slate-600 mt-1 flex items-center gap-1 font-medium">
                  <MapPin className="w-3.5 h-3.5 text-[#FFBA00] shrink-0" /> {srv.address}
                </p>
              </div>
              ...
              <div className="pt-3 border-t border-slate-100 flex items-center justify-between">
                ...
                <a
                  href={`tel:${srv.phone}`}
                  className="px-3 py-1.5 rounded-xl bg-[#FFBA00] text-black font-black text-xs flex items-center gap-1 hover:bg-[#FF9F00] shadow-sm"
                >
