                  ...
                  >
                    <div className="space-y-1.5">
                      <div className="flex flex-wrap items-center gap-2">
                        ...
                        {isSelectedActive && (
                          <span className="px-2 py-0.5 rounded-full bg-[#FFBA00] text-black text-[9px] font-black uppercase shadow-sm">Active Plan</span>
                        )}
                      </div>
                      <div className="flex flex-wrap items-center gap-1.5 text-sm sm:text-lg font-black font-outfit text-slate-900">
                        <span className="truncate max-w-[140px] sm:max-w-none">{trip.origin}</span>
                        <ArrowRight className="w-4 h-4 text-[#FFBA00] shrink-0" />
                        <span className="text-[#FFBA00] font-black truncate max-w-[140px] sm:max-w-none">{trip.destination}</span>
                      </div>
                    </div>

                    <div className="flex flex-wrap items-center gap-2 sm:gap-4 text-[10px] sm:text-xs font-bold text-slate-700">
                      <span className="flex items-center gap-1 bg-slate-50 px-2 py-1 rounded-lg border border-slate-200">
                        <Calendar className="w-3 h-3 sm:w-3.5 sm:h-3.5 text-slate-400" /> {trip.duration}
                      </span>
                      <span className="flex items-center gap-1 bg-slate-50 px-2 py-1 rounded-lg border border-slate-200">
                        <Wallet className="w-3 h-3 sm:w-3.5 sm:h-3.5 text-slate-400" /> ₹{trip.budget?.toLocaleString()}
                      </span>
                      <span className="text-emerald-700 font-extrabold bg-emerald-100 px-2 py-1 rounded-full text-[10px] sm:text-xs">Safety {trip.safety_score}/100</span>

                      <button
                        className={`px-3 py-1.5 rounded-xl text-[10px] sm:text-xs font-black transition flex items-center gap-1 min-h-[36px] active:scale-95 ${
                          isExpanded ? 'bg-[#FFBA00] text-black shadow-md' : 'bg-slate-100 border border-slate-200 text-slate-800 hover:border-[#FFBA00]'
                        }`}
                      >
                  ...
                  {isExpanded && (
                    <div className="p-4 sm:p-6 border-t border-slate-200 bg-slate-50/60 text-slate-900 space-y-6 sm:space-y-8 animate-fadeIn">
                      ...
                      <div className="p-4 rounded-2xl bg-white border border-slate-200 space-y-1 shadow-sm">
                        <span className="text-[10px] font-black uppercase text-[#FFBA00] tracking-wider">AI Route Concierge Summary</span>
                        <p className="text-xs text-slate-700 font-medium leading-relaxed">{trip.trip_summary}</p>
                      </div>
